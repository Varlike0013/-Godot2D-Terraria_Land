...
func load_items_by_json() -> Dictionary: ## 加载物品JSON 已弃用
...
func get_item(new_id:int,new_qua:int=1)->Item:
	if new_id == -1:
		return null
	var info_row:TableItemRow = DB_ITEM.get_row(new_id)
	if info_row.type == Item.ItemType.MATERIALS or info_row.type == Item.ItemType.Coin:
		var item:Item = Item.new(new_id,new_qua,info_row)
		return item
	#elif item_type == Item.ItemType.TOOL or item_type == Item.ItemType.WEAPONTOOL:
		#var item:Tool = Tool.new(new_id,item_info.get(ItemInfoType.Texture2d),item_info.get(ItemInfoType.Type),item_info.get(ItemInfoType.Name),new_qua)
		#item.update_info(item_info.get(ItemInfoType.ToolEfc),item_info.get(ItemInfoType.ToolDurability))
		#return item
	#elif item_type == Item.ItemType.HEAD:
		#var item:Armor = Armor.new(new_id,item_info.get(ItemInfoType.Texture2d),item_info.get(ItemInfoType.Type),item_info.get(ItemInfoType.Name),new_qua)
		...
		#var item:Weapon = Weapon.new(new_id,item_info.get(ItemInfoType.Texture2d),item_info.get(ItemInfoType.Type),item_info.get(ItemInfoType.Name),new_qua)
		#item.update_info()
		#return item
	#elif item_type == Item.ItemType.ACCESSORY:
		#var item:Accessory = Accessory.new(new_id,item_info.get(ItemInfoType.Texture2d),item_info.get(ItemInfoType.Type),item_info.get(ItemInfoType.Name),new_qua)
		#item.update_info()
		#return item
	return null
